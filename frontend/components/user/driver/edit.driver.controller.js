(function () {
    ...
    angular.module('fleet')
        .controller('EditDriverController', controller);

    controller.$inject = ['$scope', '$mdToast', '$mdDialog', '$mdStepper', '$state', '$stateParams', 'DriverService', 'PreferencesService', 'ProfileAvatars'];

    /*@ngInject */
    function controller($scope, $mdToast, $mdDialog, $mdStepper, $state, $stateParams, DriverService, PreferencesService, ProfileAvatars) {
        ...
        function save() {            
            ...
            DriverService.updateDriver($scope.driver).then(function (response) {
                    if (response.data.success === true) {
                        $mdToast.show($mdToast.simple()
                            .textContent(response.data.msg ? response.data.msg : "Driver details updated successfully!")
                            .position("bottom right").parent(document.body));
                        $state.go('main.driver.view');
                    } else {
                        $mdToast.show($mdToast.simple()
                            .textContent(response.data.msg ? response.data.msg : "Could not update driver.")
                            .position("bottom right").parent(document.body));
                    }
                },
                function (data) {});
        ...
        function initialise() {
            $scope.driver.dob = new Date($scope.driver.dob);
            $scope.driver.issueDate = new Date($scope.driver.driverLicense.issueDate);
            $scope.driver.expiryDate = new Date($scope.driver.driverLicense.expiryDate);
            $scope.driver.bloodGroup = $scope.driver.driverPersonal.bloodGroup;
            $scope.driver.address = $scope.driver.driverPersonal.address;
            $scope.driver.phoneNumber = parseInt($scope.driver.driverPersonal.phone);
            $scope.driver.emergencyNumber = parseInt($scope.driver.driverPersonal.emergencyNo);
            $scope.driver.placeOfIssue = $scope.driver.driverLicense.placeOfIssue;
            $scope.driver.emailId = $scope.driver.driverPersonal.emailId;
            $scope.driver.licenseNumber = $scope.driver.driverLicense.licenseNumber;
            $scope.driver.vehicleClass = $scope.driver.driverLicense.vehicleClass;

            delete $scope.driver['driverLicense'];
            delete $scope.driver['driverPersonal'];
            delete $scope.driver['driverSkills'];
        }
