...
QueryBuilder.prototype.checkFilters = function(filters) {
    ...
    filters.forEach(function(filter, i) {
        ...
        if (definedFilters.indexOf(filter.id) != -1) {
			//making filter id check more forgivable 
			return;
...
QueryBuilder.prototype.addGroup = function(parent, addRule, data, flags) {
	...
	if(parent.rules.length==2){
		//alert("Cannot add any more groups. Remove previous rule/group and nest further.");
	}else{
...
QueryBuilder.prototype.addRule = function(parent, data, flags) {
	...
	//edited
	if(parent.rules.length==2){
		//alert("Restricted to create fully parenthesized expression.");
	}else{
    ...
    var out = (function parse(group) {
        ...
        group.each(function(model) {
            ...
            var rule = {
				...
				parentKey:model.filter.optgroup,
                value: value
            };
...
<dl id="{{= it.group_id }}" flex class="card rules-group-container"> \
  <dt class="rules-group-header"> \
    <div class="btn-group pull-right group-actions"> \
      <button class="md-button md-fab md-mini custom-margin" data-add="rule" style="background:yellowgreen"> \
        <md-icon class="material-icons">{{= it.icons.add_rule }}</md-icon> {{= it.lang.add_rule }} \
      </button> \
      {{? it.settings.allow_groups===-1 || it.settings.allow_groups>=it.level }} \
        <button class="md-button md-fab md-mini custom-margin" data-add="group" style="background:yellowgreen"> \
          <md-icon class="material-icons">{{= it.icons.add_group }}</md-icon> {{= it.lang.add_group }} \
        </button> \
      {{?}} \
      {{? it.level>1 }} \
        <button class="md-button md-fab md-mini custom-margin" data-delete="group" style="background:red"> \
          <md-icon class="material-icons">{{= it.icons.remove_group }}</md-icon> {{= it.lang.delete_group }} \
        </button> \
      {{?}} \
    </div> \
...
<li id="{{= it.rule_id }}" class="rule-container"> \
  <div class="rule-header"> \
    <div class="btn-group pull-right rule-actions"> \
      <button class="md-button md-fab md-mini custom-margin" data-delete="rule" style="background:red"> \
        <md-icon class="material-icons">{{= it.icons.remove_rule }}</md-icon> {{= it.lang.delete_rule }} \
      </button> \
    </div> \
  </div> \
  {{? it.settings.display_errors }} \
...
{{ var optgroup = null; }} \
<select  onmousedown="if(this.options.length>5){this.size=5;}"  onchange=""this.size=0;"" onblur="this.size=0;" class="form-control" name="{{= it.rule.id }}_filter"> \
...
{{ var optgroup = null; }} \
<select  onmousedown="if(this.options.length>5){this.size=5;}"  onchange="this.size=0;" onblur="this.size=0;" class="form-control" name="{{= it.rule.id }}_operator"> \
